...
Coverage: **250 of the book's 308 studies are shipped** (`src/content/studies/`); 231 pass `verify_studies.py`. The 19 that fail all hit the same **open bug**: inline side-lines (a side variation woven into the move list) are merged into a variant with a wrong parent link, so part of the move-tree is unreachable (e.g. study 88's `main.9`). Fixing `parse_study.py`'s inline-side-line handling is the next big lever — it would also unblock study 99 and several still-unshipped studies. The ~58 still-unshipped fail for: classifier FEN errors (wrong piece/square — need diagram overrides), prose-embedded moves not tabulated, or "same solution as previous study" write-ups. `data/build_summary.json` records the last `--all` run's per-study status, FEN source, and `kings_ok`/`material_ok` flags.
