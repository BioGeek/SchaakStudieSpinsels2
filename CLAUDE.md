...
`.github/workflows/deploy.yml` builds and publishes to **GitHub Pages** on every push to `master` (custom domain `schaakstudiespinsels2.be` via `public/CNAME`). The workflow has three jobs: **`verify`** → **`build`** → **`deploy`**.

**SEO / sharing / analytics:** `BaseLayout.astro` emits a per-page `<meta description>`, `canonical`, `nl`/`en`/`x-default` `hreflang` alternates, and Open Graph + Twitter Card tags (default image `public/og-default.png`, a 1200×630 cover card regenerated by `scripts/generate_og_image.py`). `@astrojs/sitemap` emits `sitemap-index.xml` with hreflang links; `public/robots.txt` points at it. `GoogleAnalytics.astro` injects GA4 **only when `PUBLIC_GA_ID` is set** (the deploy `build` step reads it from the `PUBLIC_GA_ID` repo variable); unset → no tag. Note: GA loads without a consent gate — for the EU/.be audience a cookie-consent banner (or GA Consent Mode / a cookieless alternative) is the compliant path. The `verify` gate runs `uv run python scripts/verify_studies.py --fail-only` (fast; no PDF/network, just `python-chess`) and `build` `needs: verify`, so a study whose move-tree is illegal/inconsistent blocks the deploy — `astro build` alone only validates the Zod schema *shape*, not chess validity. `verify_studies.py` exits non-zero when any study fails. The tablebase audit (`tablebase_audit.py`) is intentionally **not** a CI gate (needs the Lichess API — network/rate-limited); run it manually/periodically.
